...
var FirebaseNamespaceInternals = (function () {
    ...
    FirebaseNamespaceInternals.prototype.initializeApp = function (options, appName) {
        ...
        if (typeof appName !== 'string' || appName === '') {
            throw new error_1.FirebaseAppError(error_1.AppErrorCodes.INVALID_APP_NAME, "Invalid Firebase server name \"" + appName + "\" provided. App name must be a non-empty string.");
        }
        else if (appName in this.apps_) {
            if (appName === DEFAULT_APP_NAME) {
                throw new error_1.FirebaseAppError(error_1.AppErrorCodes.DUPLICATE_APP, 'The default Firebase server already exists. This means you called initializeApp() ' +
                    'more than once without providing an server name as the second argument. In most cases ' +
                    'you only need to call initializeApp() once. But if you do want to initialize ' +
                    'multiple apps, pass a second argument to initializeApp() to give each server a unique ' +
                    'name.');
            }
            else {
                throw new error_1.FirebaseAppError(error_1.AppErrorCodes.DUPLICATE_APP, "Firebase server named \"" + appName + "\" already exists. This means you called initializeApp() " +
                    'more than once with the same server name as the second argument. Make sure you provide a ' +
    ...
    FirebaseNamespaceInternals.prototype.app = function (appName) {
        ...
        if (typeof appName !== 'string' || appName === '') {
            ...
            if (appName === DEFAULT_APP_NAME) {
                errorMessage = 'The default Firebase server does not exist. ';
            }
            else {
                errorMessage = "Firebase server named \"" + appName + "\" does not exist. ";
            }
    ...
    FirebaseNamespaceInternals.prototype.removeApp = function (appName) {
        if (typeof appName === 'undefined') {
            throw new error_1.FirebaseAppError(error_1.AppErrorCodes.INVALID_APP_NAME, "No Firebase server name provided. App name must be a non-empty string.");
        }
     ...
     * @param {AppHook} [appHook] Optional callback that handles server-related events like server creation and deletion.
    ...
    FirebaseNamespaceInternals.prototype.registerService = function (serviceName, createService, serviceProperties, appHook) {
        ...
        // or uses the default server if no FirebaseApp instance is provided
    ...
    /**
     * Calls the server hooks corresponding to the provided event name for each service within the
     * provided FirebaseApp instance.
     *
     * @param {FirebaseApp} app The FirebaseApp instance whose server hooks to call.
     * @param {string} eventName The event name representing which server hooks to call.
     */
...
var FirebaseNamespace = (function () {
    ...
    function FirebaseNamespace() {
        // Hack to prevent Babel from modifying the object returned as the default fireAdmin namespace.
