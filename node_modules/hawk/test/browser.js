...
describe('Browser', function () {
    ...
    it('should generate a bewit then successfully authenticate it', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            var bewit = Browser.client.bewit('http://example.com/resource/4?a=1&b=2', { credentials: credentials1, ttlSec: 60 * 60 * 24 * 365 * 100, ext: 'some-server-data' });
            req.url += '&bewit=' + bewit;
            ...
            Hawk.uri.authenticate(req, credentialsFunc, {}, function (err, credentials2, attributes) {
                ...
                expect(attributes.ext).to.equal('some-server-data');
                done();
            });
        });
    });
    ...
    it('generates a header then successfully parse it (configuration)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            req.authorization = Browser.client.header('http://example.com:8080/resource/4?filter=a', req.method, { credentials: credentials1, ext: 'some-server-data' }).field;
            ...
            Hawk.server.authenticate(req, credentialsFunc, {}, function (err, credentials2, artifacts) {
                ...
                expect(err).to.not.exist();
                expect(credentials2.user).to.equal('steve');
                expect(artifacts.ext).to.equal('some-server-data');
                done();
            });
        });
    });
    ...
    it('generates a header then successfully parse it (node request)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            var reqHeader = Browser.client.header('http://example.com:8080/resource/4?filter=a', req.method, { credentials: credentials1, ext: 'some-server-data', payload: payload, contentType: req.headers['content-type'] });
    ...
    it('generates a header then successfully parse it (time offset)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            req.authorization = Browser.client.header('http://example.com:8080/resource/4?filter=a', req.method, { credentials: credentials1, ext: 'some-server-data', localtimeOffsetMsec: 100000 }).field;
            expect(req.authorization).to.exist();
    ...
    it('generates a header with stale ts and successfully authenticate on second call', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            var header = Browser.client.header('http://example.com:8080/resource/4?filter=a', req.method, { credentials: credentials1, ext: 'some-server-data' });
            ...
            Hawk.server.authenticate(req, credentialsFunc, {}, function (err, credentials2, artifacts2) {
                ...
                req.authorization = Browser.client.header('http://example.com:8080/resource/4?filter=a', req.method, { credentials: credentials2, ext: 'some-server-data' }).field;
                expect(req.authorization).to.exist();
                ...
                Hawk.server.authenticate(req, credentialsFunc, {}, function (err, credentials3, artifacts3) {
                    ...
                    expect(artifacts3.ext).to.equal('some-server-data');
                    done();
                });
            });
        });
    ...
    it('generates a header then successfully parse it (with hash)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            req.authorization = Browser.client.header('http://example.com:8080/resource/4?filter=a', req.method, { credentials: credentials1, payload: 'hola!', ext: 'some-server-data' }).field;
            Hawk.server.authenticate(req, credentialsFunc, {}, function (err, credentials2, artifacts) {
    ...
    it('generates a header then successfully parse it (server)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            req.authorization = Browser.client.header('http://example.com:8080/resource/4?filter=a', req.method, { credentials: credentials1, ext: 'some-server-data', app: 'asd23ased' }).field;
            Hawk.server.authenticate(req, credentialsFunc, {}, function (err, credentials2, artifacts) {
    ...
    it('generates a header then successfully parse it (server, dlg)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            req.authorization = Browser.client.header('http://example.com:8080/resource/4?filter=a', req.method, { credentials: credentials1, ext: 'some-server-data', app: 'asd23ased', dlg: '23434szr3q4d' }).field;
            Hawk.server.authenticate(req, credentialsFunc, {}, function (err, credentials2, artifacts) {
